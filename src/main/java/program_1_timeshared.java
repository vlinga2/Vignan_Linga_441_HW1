...
public class program_1_timeshared {
    ...
    private static String fileName = "configuration_1.";
    private static Config data =  ConfigFactory.load("configuration_1.conf");
    ...
    private static Scheduler vmScheduler;
     static PeProvisionerSimple peProvisionerSimple;
    ...
    public static void main(String[] args) throws Exception {
        ...
        vmScheduler = Scheduler.TIMESHARED;
        ...
        if (datacenters.get(0).getHostList().size()>EdgeSwitch.PORTS * RootSwitch.PORTS * AggregateSwitch.PORTS){
            for(Host i:datacenters.get(0).getHostList()){
                ...
                //datacenters.get(0).pro
                datacenters.get(0).addSwitch(new EdgeSwitch(cloudSim,datacenters.get(1)));
            }
        }

        brokerPolicy1 brokerPolicy = new brokerPolicy1(cloudSim,"broker1");
        ...
        //We can add network delays. (latency or limit bandwidth)
    ...
    static UtilizationModel getUtilizationModel (){
        if (utilizationModel == null) return new UtilizationModelFull();
